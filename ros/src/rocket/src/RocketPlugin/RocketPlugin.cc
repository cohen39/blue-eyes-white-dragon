  ...
  _tf_angvel(tf_angvel_functions())
{
  ...
  _rocket_aero_forces.arg (0,x); //Set address of rocket state for each eom
  _rocket_aero_moments.arg(0,x); //Set address of rocket state for each eom
  _rocket_prop_forces.arg (0,x); //Set address of rocket state for each eom
  _rocket_prop_moments.arg(0,x); //Set address of rocket state for each eom

  //TODO implement control algorithm that determines u. Make u a private member variable and update it in update().
  double u[4]   = {0,0,0,0};     //Set input to zero for now
